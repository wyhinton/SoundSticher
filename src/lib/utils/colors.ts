...
export function getNextAvailableColor(sections: Section[]): AbletonColor {
  ...
  // Fallback: if all colors are used, just reuse the first one
  return ABLETON_COLORS[0]!; // Use non-null assertion since we know the array has elements
}

export function getDefaultColor(): AbletonColor {
  return ABLETON_COLORS[0]!; // Use non-null assertion since we know the array has elements
}
